...
| --- | --- | --- | --- |
| **Web v1.3.0** | 自己的电脑或自建服务器 | 平台链接解析、清晰度选择、下载合并、暂停续传、浏览器保存 | ZIP / tar.gz 部署包；需要 Python、FFmpeg 和适用的 JS 运行环境 |
| **Android v1.4.0-alpha.1** | 手机本机 | 内置解析／下载／合并引擎、持久任务、系统文件导出 | 源码及构建说明；本地开发 APK 已通过模拟器测试。原生依赖对应源码与许可告知补齐前，暂不公开附加 APK |
...
- **Web：**114 项 Python 测试、前端及多语言检查；真实本地 HTTP/HLS 续传；响应式浏览器流程；Windows 解压部署包启动。
- **Android：**ARM64/x86_64 构建、APK 签名校验、Android 16 x86_64 模拟器 3 项集成测试，包含安装包内 yt-dlp 下载本地 HTTP 素材及 FFmpeg 合并后的音画检查。
